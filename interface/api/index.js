...
let reactorDB = new node_json_db_1.JsonDB(new JsonDBConfig_1.Config("reactorDataBase", true, true, '/'));
...
function integratedError(error, ID) {
    let intErrTotal;
...
function derivedError(error, ID) {
    let energyHistory;
...
function updateReactor(reactorData) {
...
function updatePIDError(reactorData) {
    reactorDB.push(`/${reactorData.ID}/reactorError`, {
        error: reactorData.rfUsage
    });
}
function newReactor(ID, NAME, WORLD) {
